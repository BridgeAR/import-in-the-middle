...
/** @typedef {ReturnType<typeof parse>} EsmParseResult */
...
/**
 * @param {string} moduleSource The source code of the module to parse.
 * @returns {EsmParseResult} The lexer result.
 */
export function parseEsm (moduleSource) {
 ...
 * @param {{ parsed?: EsmParseResult }} [capture] Receives the lexer result.
 * @returns {LexedExports}
 */
export function lexEsm (moduleSource, parentURL, parsed, capture) {
  const legacy = parentURL === undefined
  const exportNames = legacy ? new Set() : []
  let starReexports
  const parsedResult = parsed ?? parse(moduleSource)
  if (capture !== undefined) capture.parsed = parsedResult
  const [, exports, , hasModuleSyntax] = parsedResult
