 ...
 */
function register (name, binder, specifier, getOrBridgeRequire, legacySpecifier) {
  if (arguments.length === 5) {
    registerLegacy(name, binder, specifier, getOrBridgeRequire, legacySpecifier)
    return
  }
  ...
  const proxy = new Proxy(namespace, binder.hasLiveBindings() ? liveProxyHandler : proxyHandler)
  const cacheBridge = bridgeRequire ? iitmGlobal[cacheBridgeKey] : undefined
...
class ModuleBinder {
   ...
   */
  static bindInPlace (name, specifier, keys, values, read, write) {
    ...
    register(name, binder, specifier, true)
  }
  ...
  /**
   * @returns {boolean} Whether reads must follow a native mutable binding.
   */
  hasLiveBindings () {
