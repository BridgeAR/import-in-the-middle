...
/** @typedef {Parameters<typeof lexEsm>[2]} EsmParseResult */
/** @typedef {{ source?: string, parsed?: EsmParseResult }} EsmCapture */

let parserInitialized = false
let stripTypeScriptTypes

/**
 * @param {import('node:module').ModuleSource} source Loader-provided module source.
 * @returns {string} The source decoded as UTF-8.
 */
 ...
 * @param {EsmCapture} [capture] Receives the root ESM source and lexer result.
 *
 ...
 * module.
 */
export function * getModuleExports (url, context, parsed, capture) {
  const cached = esmExportsCache.get(url)
  if (cached !== undefined && capture === undefined) {
    return cached
  }
  ...
  try {
    ...
    const { exportNames, starReexports, hasModuleSyntax } = lexEsm(source, url, parsed, capture)
    const moduleExports = starReexports === undefined ? { exportNames } : { exportNames, starReexports }

    if (moduleFormat === 'module') {
      if (format === 'module' && capture !== undefined) {
